...
# =============================================================================
# Installs the Caribou Hub Logs & OTA companion service on a Raspberry Pi.
#
...
read -p "  Caribou Hub URL [${DEFAULT_HUB_URL}]: " HUB_URL
...
if [ -z "$API_KEY" ]; then
    ...
    echo -e "${DIM}  Generate one from Caribou Hub → Settings → API Keys${NC}"
    exit 1
fi
