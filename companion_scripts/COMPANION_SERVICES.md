...
```
--server         Caribou Hub server URL (required)
...
All configuration is stored in `/home/alexd/caribou/forwarder.env`:

```bash
WEB_SERVER_URL=https://your-caribou-hub.com
...
```
--hub-url          Caribou Hub server URL (required)
--api-key          API key for authentication (required)
...
--log-store-dir    Local directory for cached FC log files (default: /var/lib/caribou/fc_logs/)
...
curl -I https://your-caribou-hub.com

# Verify forwarder.env
cat /home/alexd/caribou/forwarder.env

# Ensure WEB_SERVER_URL is the base URL (no trailing /api/rest/...)
# Ensure API_KEY matches the drone's key in the Caribou Hub web UI
```
