# Caribou Camera Feed Application Architecture

## Overview

The Camera Feed application provides live video streaming and gimbal control for the built-in SIYI A8 mini gimbal camera on Caribou aircraft. Unlike payload devices that occupy payload slots (C1/C2/C3), the A8 mini is integrated directly into the airframe and connects to the companion computer via the onboard Ethernet switch.

This document describes how the Camera Feed application fits into the existing Caribou Hub architecture, leveraging the same patterns established for telemetry forwarding and point cloud visualization.
...
The A8 mini camera sits on the same `192.168.144.0/24` network as the companion computer. The companion acts as a bridge, receiving commands from Caribou Hub over the internet and forwarding them to the camera over the local Ethernet network.
...
A Python service running on the Raspberry Pi manages the TCP connection to the A8 mini and exposes a WebSocket interface for Caribou Hub.
...
2. Re-encodes or relays the stream to Caribou Hub via WebRTC or HLS
...
- Upload segments to Caribou Hub storage
...
### 3. Caribou Hub: Camera Feed Application

The Camera Feed app in Caribou Hub provides the user interface for viewing video and controlling the gimbal.
...
The Camera Feed app follows the same patterns as other Caribou Hub applications:
...
A simple HTTP server exposes the HLS segments, which the companion's WebSocket bridge uploads to Caribou Hub or serves directly if the operator is on the same network.

### Integration with Camera Feed App

The Camera Feed application in Caribou Hub uses HLS.js to play the stream:
...
2. React component sends WebSocket message to Caribou Hub server
...
3. HLS segments uploaded to Caribou Hub storage (or served locally)
...
3. Caribou Hub existing WebSocket architecture (telemetry_forwarder.py)
4. Caribou Hub point cloud visualization patterns (LidarApp.tsx)
