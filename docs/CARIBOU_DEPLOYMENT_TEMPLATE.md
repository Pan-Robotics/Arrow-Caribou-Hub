# Caribou Deployment Template

This template shows how to deploy custom payload parsers directly on Caribou edge devices for autonomous data ingestion and forwarding to the Caribou Hub.

## Overview

When deploying to Caribou devices, your parser runs as a standalone Flask/FastAPI server that:
...
3. Forwards parsed data to Caribou Hub REST endpoint
...
#!/usr/bin/env python3
"""
Caribou Edge Parser Service
Autonomous payload parser for edge deployment on Caribou devices

This service receives raw sensor data, parses it using your custom parser,
and forwards the parsed data to Caribou Hub for visualization.
"""
...
# ============================================================================
# DATA SCHEMA
# Define the output structure that Caribou Hub expects
# ============================================================================
...
def ingest_payload():
    """
    Receive raw payload, parse it, and forward to Caribou Hub
    ...
    try:
        ...
        # Forward to Caribou Hub
        hub_endpoint = f"{CARIBOU_HUB_URL}/api/rest/payload/{APP_ID}/ingest"
        ...
        if response.status_code == 200:
            print(f"[INFO] Successfully forwarded to Caribou Hub")
            return jsonify({
                "success": True,
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Forwarding to: {CARIBOU_HUB_URL}/api/rest/payload/{APP_ID}/ingest")
...
1. Copy the template above to your Caribou device
...
Create `/etc/systemd/system/caribou-parser.service`:

```ini
[Unit]
Description=Caribou Edge Parser Service
...
sudo systemctl enable caribou-parser
sudo systemctl start caribou-parser
sudo systemctl status caribou-parser
```
...
```
http://<caribou-device-ip>:5000/ingest
```
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/caribou-parser.log'),
...
```bash
sudo systemctl status caribou-parser
sudo journalctl -u caribou-parser -f
```
...
```bash
tail -f /var/log/caribou-parser.log
```
...
### Data Not Reaching Caribou Hub

- Verify `CARIBOU_HUB_URL` and `APP_ID` are correct
- Check network connectivity: `curl -I $CARIBOU_HUB_URL`
...
def forward_scan(scan_data):
    requests.post(
        "http://caribou-device:5000/ingest",
        json={
            "points": scan_data,
            "timestamp": datetime.utcnow().isoformat() + "Z"
        }
...
The parser service would then transform and forward to Caribou Hub for visualization.
