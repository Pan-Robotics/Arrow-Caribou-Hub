...
The Remote Logs tab provides a live terminal view of journalctl output from any systemd service running on the companion computer. The service dropdown is **populated dynamically from the drone's diagnostics payload** — it reads the same `services` map the Diagnostics tab renders (via `trpc.diagnostics.latest`, refreshed every ~15 s), so it always lists exactly the systemd units the companion actually monitors, with no hard-coded service list to maintain. It defaults to `logs-ota` (the unit forwarding this very stream), falls back to another reported unit if the selected one disappears, and is disabled with a "Waiting for drone diagnostics…" placeholder until the first diagnostics tick arrives. When the user selects a service and clicks **Start Streaming**, the browser emits a `subscribe_logs` Socket.IO event followed by a `log_stream_request` event with `action: "start"`.
...
**`DiagnosticsCollector`** gathers system health metrics using `psutil` (CPU, memory, disk, temperature, network) and checks the status of a **configurable set of monitored systemd units** via `systemctl is-active` — a sensible default (e.g. `csu`, `caribou-can`, `logs-ota`, `go2rtc`, `tailscale-funnel`, `tailscaled`) plus any added per-fleet via the `MONITORED_SERVICES_EXTRA` / `MONITORED_SERVICES` env vars (set in `~/caribou-logs-ota.env`). The resulting `services` map drives both the Diagnostics tab and the Remote Logs dropdown, so adding a payload service (winch, cargo, …) on the Pi surfaces in the UI automatically — no Hub-side change.
...
|------|---------|-------------|
| `--hub-url` | (required) | Caribou Hub URL |
...
| `--log-store-dir` | `/var/lib/caribou/fc_logs/` | Local directory for cached FC log files |
...
**Remote Logs Tab** provides a terminal-style log viewer. A dropdown — **populated from the units the drone reports in its diagnostics `services` map, not a hard-coded list** — selects the target service; it defaults to `logs-ota`, falls back if the selected unit stops being reported, and shows "Waiting for drone diagnostics…" (disabled) until the first report arrives. Start/Stop buttons control the stream. Log lines appear in a monospace, dark-background container with auto-scroll. A "Clear" button resets the buffer. Lines arrive via Socket.IO `log_stream` events.
