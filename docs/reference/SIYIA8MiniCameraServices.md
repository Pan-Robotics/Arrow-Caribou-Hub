...
This directory contains Python services for controlling the SIYI A8 mini gimbal camera and streaming video to Caribou Hub.
...
```bash
scp -r companion_scripts/ pi@<companion-ip>:/home/pi/caribou/
```
...
--hub-url       Caribou Hub WebSocket URL
...
The controller broadcasts camera status to Caribou Hub every second:
...
2. Verify WebSocket connection to Caribou Hub
