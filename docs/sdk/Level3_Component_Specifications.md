...
drivers:
  plugin_directory: /opt/caribou/payload-drivers
...
logging:
  level: INFO
  file: /var/log/caribou/payload-manager.log
  max_size: 10MB
  rotation: 7  # days
```
...
The routing daemon is configured via a JSON file (`/etc/caribou/mavlink-routerd.conf`):
...
```json
{
  ...
  "logging": {
    "enabled": true,
    "directory": "/var/log/caribou/mavlink",
    "max_file_size": "100MB",
    "rotation": "daily"
  }
}
...
The web server is configured via environment variables and a JSON configuration file (`/etc/caribou/web-server.conf`):
...
```json
{
  "server": {
    ...
    "ssl": {
      "enabled": true,
      "cert": "/etc/caribou/ssl/server.crt",
      "key": "/etc/caribou/ssl/server.key"
    }
  },
  "authentication": {
    "jwt_secret": "CHANGE_THIS_SECRET",
    "token_expiry": "24h",
    "user_database": "/etc/caribou/users.db"
  },
  "video": {
...
- `caribou/dronecan/in/<message_type>/<node_id>` - Incoming messages from DroneCAN devices
- `caribou/dronecan/out/<message_type>/<node_id>` - Outgoing messages to DroneCAN devices

For example, a rangefinder measurement from node 42 would be published to `caribou/dronecan/in/uavcan.equipment.range_sensor.Measurement/42` with a JSON payload containing the measurement data.
...
The DroneCAN service is configured via `/etc/caribou/dronecan.conf`:

```yaml
can_interface: can0
node_id: 100
node_name: "Caribou Companion Computer"
...
mqtt:
  ...
  topic_prefix: caribou/dronecan

logging:
  level: INFO
  file: /var/log/caribou/dronecan.log
```
...
**Purpose**: The Caribou SDK defines a custom MAVLink dialect that extends the standard MAVLink message set with payload-specific messages and commands.

#### Message Definitions

The dialect is defined in XML format (`caribou.xml`) and compiled into C/C++ headers using the MAVLink code generator. Example message definitions:

```xml
<message id="50000" name="PAYLOAD_STATUS">
  <description>Status information from a Caribou payload</description>
...
The custom dialect is integrated into the flight controller firmware by including the generated header files and registering message handlers. For ArduPilot, this involves creating a new library in `libraries/AP_Caribou/` that implements the Caribou-specific logic. For PX4, a new module is added to `src/modules/caribou/`.

The flight controller firmware is modified to:
1. Forward Caribou-specific messages between the companion computer and ground station
2. Respond to Caribou commands (e.g., MAV_CMD_PAYLOAD_ACTIVATE) by sending corresponding messages to the companion computer
3. Log Caribou telemetry to onboard flash storage for post-flight analysis
...
**Connector Interface**: The payload connects to the Caribou 10-pin connector via a custom PCB that breaks out the Ethernet differential pairs (TX+/TX-, RX+/RX-) to an RJ45 jack or direct PHY connection. The PCB includes:
...
**Payload Service**: A Python application that implements the Caribou payload protocol. It listens on HTTP port 8080 for control commands, publishes telemetry via MQTT or HTTP POST, and streams sensor data via UDP or RTSP. The service uses systemd for automatic startup and restart on failure.
...
namespace CaribouCameraPlugin
{
    public class CameraPlugin : Plugin
    {
        public override string Name => "Caribou Camera Control";
        public override string Version => "1.0";
        public override string Author => "Caribou SDK";
...
**Flight Controller Parameters**: Stored in the flight controller's EEPROM, managed via MAVLink's PARAM protocol. Examples: `CARIBOU_PAYLOAD1_EN` (enable payload 1), `CARIBOU_TELEM_RATE` (telemetry rate for Caribou messages).

**Companion Computer Parameters**: Stored in `/etc/caribou/params.yaml`, managed via the web interface or command-line tool. Examples: `payload_discovery_interval`, `video_bitrate`, `telemetry_logging_enabled`.
...
The component specifications detailed in this document provide the technical foundation for implementing and integrating with the Caribou SDK. Each component is designed with modularity, security, and performance in mind, ensuring that the system can accommodate diverse payloads while maintaining flight safety and reliability. Developers and integrators should refer to these specifications when building payload drivers, extending ground control interfaces, or customizing the companion computer software stack.
