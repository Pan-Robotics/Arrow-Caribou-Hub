...
All services run as systemd units, auto-restart on failure, and log to journald. Configuration lives in `$HOME/caribou/forwarder.env`.
...
CLI: python3 raspberry_pi_client.py \
       --server $CARIBOU_HUB_URL --drone-id $DRONE_ID \
...
CLI: python3 logs_ota_service.py \
       ...
       --log-store-dir /var/lib/caribou/fc_logs --debug
```
...
Open Caribou Hub → Drone Configuration → select or create a drone → API Keys section → click "Generate Key". Copy the key — it is shown only once.

### Step 2: Configure the .env File

On the Raspberry Pi, create `$HOME/caribou/forwarder.env`:

```
CARIBOU_HUB_URL=https://your-caribou-hub.com
CARIBOU_DRONE_ID=caribou_001
CARIBOU_API_KEY=<paste-key-here>
...
```bash
cd $HOME/caribou/companion_scripts
chmod +x install_*.sh
...
Open Caribou Hub in a browser. You should see:
...
url = os.environ["CARIBOU_HUB_URL"] + "/api/rest/payload/YOUR_APP_ID/ingest"
headers = {"x-api-key": os.environ["CARIBOU_API_KEY"], "Content-Type": "application/json"}
...
**Step 1 — Define the handler.** In `raspberry_pi_client.py`, add a method to the `CaribouHubClient` class:
...
```bash
sudo systemctl restart caribou-hub-client
```
...
```bash
sudo journalctl -u caribou-hub-client -f
```
...
| Service statuses | `systemctl is-active` for each Caribou service |
...
| Path | Purpose |
|---|---|
| `$HOME/caribou/forwarder.env` | Shared environment configuration |
| `$HOME/caribou/raspberry_pi_client.py` | Hub client script |
...
```bash
# Service management
sudo systemctl status caribou-hub-client
...
# Test Hub endpoint
curl -X POST $CARIBOU_HUB_URL/api/rest/health
```
