# Caribou SDK Developer Guide - Python Edition
## Outline (Concise & Practical)

**Version:** 1.1  
**Date:** January 2026  
**Focus:** Python-only, based on actual Caribou Hub implementation

---

## 1. Introduction

### 1.1 What is Caribou SDK?

The Caribou SDK is a Python-based development toolkit that enables developers to build applications for both companion computers and payload devices in the Caribou ecosystem. It provides a standardized interface for communicating with Caribou Hub, the web-based control center that serves as the central coordination point for drone operations.

**Core Capabilities:**
- **Telemetry Forwarding**: Collect and stream flight controller data (MAVLink) and sensor data (UAVCAN) to Caribou Hub
...
**Connection Details:**
- **Wireless/Internet**: Companion ↔ Caribou Hub (HTTPS REST API, WebSocket telemetry)
...
Before developing with the Caribou SDK, ensure you have the required hardware and software components. The companion computer (Raspberry Pi) serves as your primary development target, while the flight controller provides the foundation for flight operations and telemetry.

**Hardware Requirements:**

**Companion Computer**: Raspberry Pi 4 (4GB+ RAM) or Raspberry Pi 5 recommended. Earlier models lack sufficient processing power for multi-threaded telemetry forwarding and payload management. The Pi must have WiFi or cellular connectivity for internet access to Caribou Hub.
...
**Integrated Network Switch**: Built into Caribou drone hardware, this component is pre-configured and requires no user setup. It provides Ethernet ports for payloads (C1, C2, C3) and CAN bus connectivity to the flight controller.
...
**Network Connectivity**: WiFi or cellular modem for internet access to Caribou Hub. Companion computer must maintain persistent connection for job polling and telemetry streaming. Cellular modems (4G/LTE) recommended for beyond-visual-line-of-sight (BVLOS) operations.
...
The Caribou Hub Client implements a polling-based job execution system that enables operators to remotely manage the companion computer from the Caribou Hub web interface. The client periodically checks for pending jobs, executes them locally, and reports completion status back to the Hub.

**Purpose**: Bidirectional communication between companion computer and Caribou Hub for remote management and control.
...
The Telemetry Forwarder collects real-time flight data from multiple sources (flight controller via MAVLink, battery via UAVCAN) and streams it to Caribou Hub for visualization and logging. It uses a multi-threaded architecture to handle concurrent data collection and HTTP transmission without blocking.

**Purpose**: Collect flight controller telemetry and sensor data, aggregate into unified format, and stream to Caribou Hub.
...
Flight Controller (MAVLink) ──┐
                               ├──> Telemetry Dict ──> HTTP Queue ──> Caribou Hub
...
Understanding the end-to-end workflows helps developers see how their code fits into the larger Caribou ecosystem. This section describes common user interactions and the corresponding data flows through the system.

### 3.1 User Flow: Monitoring Telemetry

An operator wants to monitor a drone's flight in real-time from the Caribou Hub web interface.

**Steps**:
1. Operator opens Caribou Hub in web browser and navigates to "Flight Telemetry" app
2. Selects drone "caribou_001" from dropdown menu
3. Hub establishes WebSocket connection to backend server
4. Backend subscribes to telemetry stream for caribou_001
...
7. Hub backend broadcasts telemetry to all connected WebSocket clients for caribou_001
...
This workflow demonstrates the pub-sub pattern used throughout Caribou: companion computers push data to Hub via REST API, Hub stores and broadcasts to all interested clients via WebSocket.
...
This workflow demonstrates the extensibility of Caribou: developers can add new sensors by following a simple pattern (read, format, POST) without modifying core SDK code.

---

## 4. API Reference (Python)

This section provides Python code examples for interacting with the Caribou Hub REST API. All examples use the `requests` library for synchronous HTTP calls. For production code, consider using `aiohttp` for async/await patterns.

### 4.1 Caribou Hub REST API
...
def get_pending_jobs(server_url, drone_id, api_key):
    """Fetch pending jobs from Caribou Hub"""
...
def post_telemetry(server_url, api_key, drone_id, telemetry_data):
    """Post telemetry data to Caribou Hub"""
...
**Goal**: Read Raspberry Pi CPU temperature and send to Caribou Hub every 5 seconds.
...
Reads Raspberry Pi CPU temp and sends to Caribou Hub
"""
...
def send_telemetry(temperature):
    """Send temperature data to Caribou Hub"""
...
**Deployment**:
1. Save as `/home/pi/caribou/cpu_temp_monitor.py`
...
**Expected Output**:
```
Starting CPU temperature monitor for caribou_001
...
**Goal**: Extend Caribou Hub Client to handle a custom "restart_service" job type.

**Hardware**: Raspberry Pi with systemd services.

**Concepts**: Extending CaribouHubClient class, subprocess execution, error handling.

```python
#!/usr/bin/env python3
"""
Extended Caribou Hub Client with custom job handler
Adds support for restarting systemd services remotely
"""

import subprocess
from typing import Dict, Optional, Tuple
from raspberry_pi_client import CaribouHubClient  # Import base class

class ExtendedCaribouHubClient(CaribouHubClient):
...
# Usage (replace CaribouHubClient with ExtendedCaribouHubClient in main())
if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Extended Caribou Hub Client')
    ...
    client = ExtendedCaribouHubClient(
...
**Testing**:
1. Create job in Hub UI with type "restart_service" and payload `{"serviceName": "caribou-telemetry"}`
2. Client will pick up job, execute `sudo systemctl restart caribou-telemetry`
3. Check logs: `sudo journalctl -u caribou-client -f`

**Sudoers Configuration** (required for passwordless sudo):
```bash
# Add to /etc/sudoers.d/caribou-client
...
Streams 2D laser scan data to Caribou Hub for real-time visualization
"""
...
async def connect():
    ...
    print("Connected to Caribou Hub")
...
async def disconnect():
    ...
    print("Disconnected from Caribou Hub")
...
Production deployments require proper service management, logging, and error handling. This section covers best practices for deploying Caribou SDK applications on companion computers.
...
Create `/etc/systemd/system/caribou-client.service`:
```ini
[Unit]
Description=Caribou Hub Client
...
Type=simple
User=pi
WorkingDirectory=/home/pi/caribou
ExecStart=/usr/bin/python3 /home/pi/caribou/raspberry_pi_client.py \
  --server https://your-hub.com \
  --drone-id caribou_001 \
  --api-key abc123 \
  --poll-interval 5
...
sudo systemctl enable caribou-client.service

# Start service now
sudo systemctl start caribou-client.service

# Check status
sudo systemctl status caribou-client.service

# View logs
sudo journalctl -u caribou-client.service -f
```

**Multiple Services**:

For a complete deployment, create separate services for each component:
- `caribou-client.service`: Job polling and execution
- `caribou-telemetry.service`: Flight controller telemetry forwarding
- `caribou-lidar.service`: LiDAR point cloud streaming (if applicable)
- `caribou-camera.service`: Camera streaming (if applicable)
...
Create `/home/pi/caribou/.env`:
```bash
# Caribou Hub Configuration
...
load_dotenv('/home/pi/caribou/.env')
...
```ini
[Service]
EnvironmentFile=/home/pi/caribou/.env
ExecStart=/usr/bin/python3 /home/pi/caribou/telemetry_forwarder.py
```

**Security Note**: Protect `.env` file from unauthorized access:
```bash
chmod 600 /home/pi/caribou/.env
```
...
def setup_logging(log_file='/var/log/caribou/client.log', level=logging.INFO):
...
sudo journalctl -u caribou-client -f

# View file logs
tail -f /var/log/caribou/client.log

# Search logs for errors
grep ERROR /var/log/caribou/*.log

# View logs from last hour
journalctl -u caribou-client --since "1 hour ago"
```
...
class Config:
    ...
    def __init__(self, config_file='/home/pi/config/caribou_config.json'):
...
Common issues and solutions for Caribou SDK deployments.

### 8.1 Connection Issues

**Problem**: Client cannot reach Caribou Hub
...
The Caribou SDK is designed for extensibility. Developers can add new capabilities without modifying core code.
...
To add custom job types, extend the `CaribouHubClient` class:
...
The Caribou SDK can integrate with external systems using standard protocols.
...
def gps_callback(msg):
    """Forward ROS GPS message to Caribou Hub"""
    data = {
        "droneId": "caribou_001",
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "position": {
...
rclpy.init()
node = rclpy.create_node('caribou_bridge')
...
```lua
-- Trigger Caribou Hub job via MAVLink command
function trigger_job()
    gcs:send_text(0, "Triggering Caribou Hub job")
...
sudo systemctl status caribou-client

# View logs (follow mode)
sudo journalctl -u caribou-client -f

# Restart service
sudo systemctl restart caribou-client

# Enable service (start on boot)
sudo systemctl enable caribou-client

# Disable service
sudo systemctl disable caribou-client
```
...
curl -X POST https://your-hub.com/api/rest/telemetry/ingest \
  ...
  -d '{"droneId":"caribou_001","timestamp":"2026-01-08T10:00:00Z","test":true}'

# Test job polling endpoint
curl "https://your-hub.com/api/trpc/droneJobs.getPendingJobs?input=%7B%22json%22%3A%7B%22droneId%22%3A%22caribou_001%22%2C%22apiKey%22%3A%22abc123%22%7D%7D"
```
...
**Application Files**:
- Client script: `/home/pi/caribou/raspberry_pi_client.py`
- Telemetry forwarder: `/home/pi/caribou/telemetry_forwarder.py`
- Custom sensor scripts: `/home/pi/caribou/sensors/`

**Configuration**:
- Environment variables: `/home/pi/caribou/.env`
- JSON configuration: `/home/pi/config/caribou_config.json`

**Logs**:
- Application logs: `/var/log/caribou/*.log`
- Systemd journal: `journalctl -u caribou-*`

**System Services**:
- Service files: `/etc/systemd/system/caribou-*.service`
- Sudoers config: `/etc/sudoers.d/caribou-client`
...
2. Validate against actual Caribou Hub implementation
