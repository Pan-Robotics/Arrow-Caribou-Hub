...
**Endpoint**: `GET http://<payload-ip>/caribou/info`
...
```json
{
  ...
  "endpoints": {
    ...
    "telemetry_topic": "caribou/payloads/camera1",
...
**Endpoint**: `GET /caribou/parameters`
...
**Endpoint**: `PUT /caribou/parameters/{name}`
...
**Topic Pattern**: `caribou/payloads/<payload_id>/<data_type>`

**Example Topics**:
- `caribou/payloads/camera1/status`
- `caribou/payloads/lidar1/pointcloud`
- `caribou/payloads/sensor1/environment`
...
### Caribou Payload Status

**Message Name**: `com.caribou.PayloadStatus`

**DSDL Definition**:

```
# com.caribou.PayloadStatus.uavcan
#
...
#include "com.caribou.PayloadStatus.hpp"

void publishPayloadStatus(uavcan::INode& node)
{
    com::caribou::PayloadStatus msg;
    msg.payload_id = 1;
    ...
    static uavcan::Publisher<com::caribou::PayloadStatus> pub(node);
    pub.broadcast(msg);
}
```
...
```python
# /opt/caribou/sdk/payload_driver.py
...
```python
# /opt/caribou/payload-drivers/http_camera.py
...
class HTTPCameraDriver(PayloadDriver):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            async with self.session.get(f"{self.base_url}/caribou/health", timeout=5) as resp:
                return resp.status == 200
        except:
            return False
    
    async def get_status(self) -> Dict[str, Any]:
        async with self.session.get(f"{self.base_url}/caribou/status") as resp:
    ...
    async def send_command(self, command: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(f"{self.base_url}/caribou/command", json=payload) as resp:
            return await resp.json()
...
Drivers are registered by placing them in `/opt/caribou/payload-drivers/` with a corresponding metadata file:
...
// Usage example
const client = new CaribouClient('https://192.168.144.15:8080');
