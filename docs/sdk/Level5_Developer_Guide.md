...
This guide provides step-by-step tutorials for developers building payloads and integrating them with the Caribou SDK. It assumes familiarity with Python or C++ programming, basic networking concepts, and Linux command-line tools. Each tutorial builds progressively, starting with simple sensors and advancing to complex multi-sensor systems.
...
- USB-to-Ethernet adapter (for testing without Caribou hardware)
- Breadboard, jumper wires, and basic electronics tools

**Caribou Test Platform** (optional but recommended):
- Caribou UAV with companion computer
...
git clone https://github.com/Pan-Robotics/caribou-sdk.git
cd caribou-sdk
...
#!/usr/bin/env python3
"""
Simple temperature sensor payload for Caribou SDK
...
class TemperatureSensorPayload:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_get('/caribou/info', self.handle_info)
        self.app.router.add_get('/caribou/health', self.handle_health)
        self.app.router.add_get('/caribou/status', self.handle_status)
        self.app.router.add_post('/caribou/command', self.handle_command)
        self.app.router.add_get('/caribou/parameters', self.handle_get_parameters)
        self.app.router.add_put('/caribou/parameters/{name}', self.handle_set_parameter)
    ...
    async def handle_info(self, request):
        ...
        return web.json_response({
            ...
            "endpoints": {
                "control_api": f"http://{request.host}/caribou",
                "telemetry_topic": "caribou/payloads/temp_sensor1"
            }
        })
        ...
        while True:
            if self.recording:
                ...
                self.mqtt_client.publish(
                    "caribou/payloads/temp_sensor1/temperature",
...
Before deploying to Caribou, test the payload locally:
...
curl http://localhost:8080/caribou/info
curl http://localhost:8080/caribou/health
curl http://localhost:8080/caribou/status

# Start recording
curl -X POST http://localhost:8080/caribou/command \
  -H "Content-Type: application/json" \
  -d '{"command": "start_recording"}'

# Change sample rate
curl -X PUT http://localhost:8080/caribou/parameters/sample_rate \
  -H "Content-Type: application/json" \
  -d '{"value": 2.0}'
```

### Step 4: Deploy to Caribou

**Configure Network**:
1. Connect Raspberry Pi to Caribou payload port C1 (IP: 192.168.144.11)
...
```ini
[Unit]
Description=Caribou Temperature Sensor Payload
...
#!/usr/bin/env python3
"""
Camera payload with video streaming for Caribou SDK
"""
...
Connect the rangefinder to the Caribou CAN bus and configure ArduPilot:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/caribou/payload.log'),
        logging.StreamHandler()
    ]
)

logger = logging.getLogger('caribou.payload')
...
# Load configuration
with open('/etc/caribou/payload-config.yaml', 'r') as f:
...
2. Check payload is responding: `curl http://192.168.144.11:8080/caribou/info`
...
- **GitHub Repository**: https://github.com/Pan-Robotics/caribou-sdk
- **Developer Forum**: [To be established]
- **Example Payloads**: https://github.com/Pan-Robotics/caribou-payload-examples
- **Documentation**: https://docs.caribou-sdk.org

---

## Conclusion

This developer guide has provided hands-on tutorials for building Caribou SDK payloads, from simple sensors to complex multi-sensor systems. By following these examples and best practices, developers can create robust, production-ready payloads that integrate seamlessly with the Caribou platform.
