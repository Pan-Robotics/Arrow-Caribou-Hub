...
describe("WebRTC Stream Registry", () => {
  ...
  it("should store stream entry with correct fields", () => {
    ...
    registry.set("caribou_001", entry);

    expect(registry.has("caribou_001")).toBe(true);
    expect(registry.get("caribou_001")?.webrtcUrl).toBe(
      "https://quiver.tail1234.ts.net/api/webrtc?src=camera"
    );
    expect(registry.get("caribou_001")?.droneId).toBe("caribou_001");
  });

  it("should overwrite existing entry on re-register (e.g. Tailscale hostname change)", () => {
    const registry = new Map<string, WebRTCStreamEntry>();

    registry.set("caribou_001", {
    ...
    expect(registry.get("caribou_001")?.registeredAt).toBe(2000);
  });
  ...
  it("should remove entry on unregister", () => {
    ...
    registry.delete("caribou_001");
    expect(registry.has("caribou_001")).toBe(false);
  ...
  it("should support multiple drones simultaneously", () => {
    ...
    registry.set("caribou_001", {
      webrtcUrl: "https://caribou1.tail1234.ts.net/api/webrtc?src=camera",
      registeredAt: Date.now(),
      droneId: "caribou_001",
    });
    registry.set("caribou_002", {
      webrtcUrl: "https://caribou2.tail5678.ts.net/api/webrtc?src=camera",
      registeredAt: Date.now(),
      droneId: "caribou_002",
    });

    expect(registry.size).toBe(2);
    expect(registry.get("caribou_001")?.webrtcUrl).toContain("caribou1");
    expect(registry.get("caribou_002")?.webrtcUrl).toContain("caribou2");
  });
});
...
describe("WebSocket Camera Stream Events (WebRTC)", () => {
  it("should format stream available event with WebRTC URL", () => {
    ...
    expect(event.drone_id).toBe("caribou_001");
  ...
  it("should use correct room name for camera subscriptions", () => {
    ...
    expect(room).toBe("camera:caribou_001");
  });
});
...
describe("WebRTC Stream Registration Request Validation", () => {
  ...
  it("should detect missing fields", () => {
    ...
    const missingWebrtcUrl = { api_key: "key", drone_id: "caribou_001" };
...
describe("Companion Computer WebRTC Registration", () => {
  it("should construct correct registration payload with WebRTC URL", () => {
    const hubUrl = "https://caribou-hub.example.com";
    const droneId = "caribou_001";
    ...
    expect(payload.drone_id).toBe("caribou_001");
