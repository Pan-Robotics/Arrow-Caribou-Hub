...
describe("Drone API Key Management", () => {
  ...
  describe("createApiKey", () => {
    it("should create an API key with description", async () => {
      ...
      const result = await createApiKey("caribou_001", "Test key");
      expect(result).toEqual(mockKey);
      expect(createApiKey).toHaveBeenCalledWith("caribou_001", "Test key");
    });
    ...
    it("should create an API key without description", async () => {
      ...
      const result = await createApiKey("caribou_002");
      expect(result).toEqual(mockKey);
      expect(createApiKey).toHaveBeenCalledWith("caribou_002");
    });

    it("should return null when database is unavailable", async () => {
      (createApiKey as any).mockResolvedValue(null);

      const result = await createApiKey("caribou_001");
  ...
  describe("getApiKeysForDrone", () => {
    it("should return all keys for a drone", async () => {
      ...
      const result = await getApiKeysForDrone("caribou_001");
      expect(result).toHaveLength(2);
      expect(result[0].droneId).toBe("caribou_001");
    ...
    it("should return empty array for drone with no keys", async () => {
      ...
      const result = await getApiKeysForDrone("caribou_999");
  ...
  describe("upsertDrone (register)", () => {
    it("should register a new drone", async () => {
      ...
      expect(result.droneId).toBe("caribou_002");
...
describe("Connection Info Generation", () => {
  ...
  function buildEnvSnippet(base: string, drone: string, key: string) {
    ...
    return [
      `CARIBOU_HUB_URL=${base}`,
      `CARIBOU_DRONE_ID=${drone}`,
      `CARIBOU_API_KEY=${key}`,
      `CARIBOU_POINTCLOUD_ENDPOINT=${base}/api/rest/pointcloud/ingest`,
      `CARIBOU_TELEMETRY_ENDPOINT=${base}/api/rest/telemetry/ingest`,
      `CARIBOU_PAYLOAD_ENDPOINT=${base}/api/rest/payload/{appId}/ingest`,
      `CARIBOU_CAMERA_STATUS_ENDPOINT=${base}/api/rest/camera/status`,
      `CARIBOU_CAMERA_STREAM_REGISTER=${base}/api/rest/camera/stream-register`,
      `CARIBOU_CAMERA_STREAM_UNREGISTER=${base}/api/rest/camera/stream-unregister`,
      `CARIBOU_CAMERA_STREAM_STATUS=${base}/api/rest/camera/stream-status/${drone}`,
      `CARIBOU_CAMERA_WHEP_PROXY=${base}/api/rest/camera/whep-proxy/${drone}`,
      `CARIBOU_FC_LOG_LIST_ENDPOINT=${base}/api/rest/logs/fc-list`,
      `CARIBOU_FC_LOG_PROGRESS_ENDPOINT=${base}/api/rest/logs/fc-progress`,
      `CARIBOU_FC_LOG_UPLOAD_ENDPOINT=${base}/api/rest/logs/fc-upload`,
      `CARIBOU_FC_LOG_UPLOAD_MULTIPART=${base}/api/rest/logs/fc-upload-multipart`,
      `CARIBOU_FC_LOG_DOWNLOAD=${base}/api/rest/logs/fc-download/{logId}`,
      `CARIBOU_FIRMWARE_PROGRESS_ENDPOINT=${base}/api/rest/firmware/progress`,
      `CARIBOU_DIAGNOSTICS_ENDPOINT=${base}/api/rest/diagnostics/report`,
      `CARIBOU_FLIGHTLOG_UPLOAD=${base}/api/rest/flightlog/upload`,
      `CARIBOU_HEALTH_ENDPOINT=${base}/api/rest/health`,
      `CARIBOU_TEST_CONNECTION=${base}/api/rest/test-connection`,
      `CARIBOU_WS_URL=${wsUrl}`,
      `CARIBOU_JOBS_ENDPOINT=${base}/api/trpc/droneJobs.getPendingJobs`,
      `CARIBOU_JOBS_ACK_ENDPOINT=${base}/api/trpc/droneJobs.acknowledgeJob`,
      `CARIBOU_JOBS_COMPLETE_ENDPOINT=${base}/api/trpc/droneJobs.completeJob`,
      `CARIBOU_JOBS_FAIL_ENDPOINT=${base}/api/trpc/droneJobs.failJob`,
      `FC_WEBSERVER_URL=http://192.168.144.20:8080`,
      `FC_LOG_STORE_DIR=/var/lib/caribou/fc_logs`,
    ].join("\n");
  }

  it("should generate correct .env snippet with all core connection fields", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_HUB_URL=https://example.com");
    expect(env).toContain("CARIBOU_DRONE_ID=caribou_001");
    expect(env).toContain("CARIBOU_API_KEY=test-key-123");
  });

  it("should include all core data pipeline endpoints", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_POINTCLOUD_ENDPOINT=https://example.com/api/rest/pointcloud/ingest");
    expect(env).toContain("CARIBOU_TELEMETRY_ENDPOINT=https://example.com/api/rest/telemetry/ingest");
    expect(env).toContain("CARIBOU_PAYLOAD_ENDPOINT=https://example.com/api/rest/payload/{appId}/ingest");
  });

  it("should include all camera pipeline endpoints", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_CAMERA_STATUS_ENDPOINT=https://example.com/api/rest/camera/status");
    expect(env).toContain("CARIBOU_CAMERA_STREAM_REGISTER=https://example.com/api/rest/camera/stream-register");
    expect(env).toContain("CARIBOU_CAMERA_STREAM_UNREGISTER=https://example.com/api/rest/camera/stream-unregister");
    expect(env).toContain(`CARIBOU_CAMERA_STREAM_STATUS=https://example.com/api/rest/camera/stream-status/${droneId}`);
    expect(env).toContain(`CARIBOU_CAMERA_WHEP_PROXY=https://example.com/api/rest/camera/whep-proxy/${droneId}`);
  });

  it("should include all logs & OTA pipeline endpoints", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_FC_LOG_LIST_ENDPOINT=https://example.com/api/rest/logs/fc-list");
    expect(env).toContain("CARIBOU_FC_LOG_PROGRESS_ENDPOINT=https://example.com/api/rest/logs/fc-progress");
    expect(env).toContain("CARIBOU_FC_LOG_UPLOAD_ENDPOINT=https://example.com/api/rest/logs/fc-upload");
    expect(env).toContain("CARIBOU_FC_LOG_UPLOAD_MULTIPART=https://example.com/api/rest/logs/fc-upload-multipart");
    expect(env).toContain("CARIBOU_FC_LOG_DOWNLOAD=https://example.com/api/rest/logs/fc-download/{logId}");
    expect(env).toContain("CARIBOU_FIRMWARE_PROGRESS_ENDPOINT=https://example.com/api/rest/firmware/progress");
    expect(env).toContain("CARIBOU_DIAGNOSTICS_ENDPOINT=https://example.com/api/rest/diagnostics/report");
  });

  it("should include system and flight analytics endpoints", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_HEALTH_ENDPOINT=https://example.com/api/rest/health");
    expect(env).toContain("CARIBOU_TEST_CONNECTION=https://example.com/api/rest/test-connection");
    expect(env).toContain("CARIBOU_FLIGHTLOG_UPLOAD=https://example.com/api/rest/flightlog/upload");
  });

  it("should include all tRPC job polling endpoints", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("CARIBOU_JOBS_ENDPOINT=https://example.com/api/trpc/droneJobs.getPendingJobs");
    expect(env).toContain("CARIBOU_JOBS_ACK_ENDPOINT=https://example.com/api/trpc/droneJobs.acknowledgeJob");
    expect(env).toContain("CARIBOU_JOBS_COMPLETE_ENDPOINT=https://example.com/api/trpc/droneJobs.completeJob");
    expect(env).toContain("CARIBOU_JOBS_FAIL_ENDPOINT=https://example.com/api/trpc/droneJobs.failJob");
  });

  it("should include FC web server configuration", () => {
    const env = buildEnvSnippet(baseUrl, droneId, apiKey);
    expect(env).toContain("FC_WEBSERVER_URL=http://192.168.144.20:8080");
    expect(env).toContain("FC_LOG_STORE_DIR=/var/lib/caribou/fc_logs");
  });
  ...
  it("should show placeholder when no active key", () => {
    const envLine = `CARIBOU_API_KEY=${undefined ? "some-key" : "<generate-an-api-key>"}`;
...
describe("DroneConfig .env UI — comprehensive endpoint reference", () => {
  ...
  it("DroneConfig.tsx envSnippet contains FC web server config", () => {
    ...
    expect(source).toContain("FC_LOG_STORE_DIR=/var/lib/caribou/fc_logs");
