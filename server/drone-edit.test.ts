...
describe("Drone Edit Features", () => {
  ...
  describe("updateDroneByDroneId", () => {
    it("should update drone name only", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", { name: "Updated Name" });

      expect(mockUpdateDroneByDroneId).toHaveBeenCalledWith("caribou_001", { name: "Updated Name" });
      expect(result).toBeDefined();
      expect(result.name).toBe("Updated Name");
      expect(result.droneId).toBe("caribou_001");
    });
    ...
    it("should update drone ID and cascade to API keys", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", { droneId: "caribou_new" });

      expect(mockUpdateDroneByDroneId).toHaveBeenCalledWith("caribou_001", { droneId: "caribou_new" });
      expect(result).toBeDefined();
      expect(result.droneId).toBe("caribou_new");
    });
    ...
    it("should update both name and droneId", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", {
        droneId: "caribou_new",
        name: "New Name",
      });
    ...
    it("should handle setting name to null (clearing it)", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", { name: null });
  ...
  describe("Drone update tRPC procedure validation", () => {
    it("should reject update when new droneId is already taken", async () => {
      ...
      const existing = await mockGetDroneByDroneId("caribou_002");
      expect(existing).toBeDefined();
      // The procedure would throw an error here
      expect(existing.droneId).toBe("caribou_002");
    });

    it("should allow update when new droneId is available", async () => {
      mockGetDroneByDroneId.mockResolvedValue(null);

      const existing = await mockGetDroneByDroneId("caribou_new");
    ...
    it("should skip droneId uniqueness check when droneId is unchanged", async () => {
      ...
      const currentDroneId = "caribou_001";
      const newDroneId = "caribou_001";
  ...
  describe("Edge cases", () => {
    ...
    it("should handle very long drone names", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", { name: longName });
    ...
    it("should handle special characters in drone ID", async () => {
      ...
      const result = await mockUpdateDroneByDroneId("caribou_001", { droneId: "drone-test_123" });
